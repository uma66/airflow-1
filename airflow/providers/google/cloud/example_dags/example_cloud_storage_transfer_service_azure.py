...
* GCP_TRANSFER_SOURCE_AZURE_CONTAINER - Microsoft Azure Blob Storage container from which files are copied.
  .. warning::
...
GCP_TRANSFER_SOURCE_AZURE_STORAGE_ACCOUNT = os.environ.get('GCP_TRANSFER_SOURCE_AZURE_STORAGE_ACCOUNT')
...
# [START howto_operator_gcp_transfer_create_job_body_azure]
azure_to_gcs_transfer_body = {
    ...
    TRANSFER_SPEC: {
        AZURE_BLOB_STORAGE_DATA_SOURCE: {
            STORAGE_ACCOUNT: GCP_TRANSFER_SOURCE_AZURE_STORAGE_ACCOUNT,
            CONTAINER: GCP_TRANSFER_SOURCE_AZURE_CONTAINER,
        },
        GCS_DATA_SINK: {BUCKET_NAME: GCP_TRANSFER_FIRST_TARGET_BUCKET},
        TRANSFER_OPTIONS: {ALREADY_EXISTING_IN_SINK: True},
    },
}
# [END howto_operator_gcp_transfer_create_job_body_azure]
...
with models.DAG(
    ...
    create_transfer_job_from_azure = CloudDataTransferServiceCreateJobOperator(
        task_id="create_transfer_job_from_azure", body=azure_to_gcs_transfer_body
    )
    ...
    wait_for_operation_to_start = CloudDataTransferServiceJobStatusSensor(
        ...
        job_name="{{task_instance.xcom_pull('create_transfer_job_from_azure')['name']}}",
        project_id=GCP_PROJECT_ID,
    ...
    list_operations = CloudDataTransferServiceListOperationsOperator(
        ...
        request_filter={
            ...
            FILTER_JOB_NAMES: ["{{task_instance.xcom_pull('create_transfer_job_from_azure')['name']}}"],
        },
    )
    ...
    delete_transfer_from_azure_job = CloudDataTransferServiceDeleteJobOperator(
        task_id="delete_transfer_from_azure_job",
        job_name="{{task_instance.xcom_pull('create_transfer_job_from_azure')['name']}}",
        project_id=GCP_PROJECT_ID,
    )
    ...
    resume_operation >> wait_for_operation_to_end >> cancel_operation >> delete_transfer_from_azure_job
    # fmt: on
